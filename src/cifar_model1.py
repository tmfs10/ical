...
class BayesianNet(mc_dropout.BayesianModule):
    ...
    def mc_forward_impl(self, input: Tensor):
        input = F.relu(self.conv1_drop(self.conv1(input)))
        input = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(input)), 2))
        input = F.relu(self.conv3_drop(self.conv3(input)))
        input = F.relu(F.max_pool2d(self.conv4_drop(self.conv4(input)), 2))
